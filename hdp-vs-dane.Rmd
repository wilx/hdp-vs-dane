...
CorrTestS <- cor.test(df$hdp, df$dane, conf.level = 0.95, method = "spearman")
CorrS <- CorrTestS$estimate
```

Pearsonův korelační koeficient mezi mezi HDP a inkasem daní je \(r(`r fmtNumM(CorrTest$parameter)`)=`r fmtNumM(Corr)`\), \({CI}_{`r attr(CorrTest$conf.int, "conf.level")`}=(`r fmtNumM(CorrTest$conf.int[1])`; `r fmtNumM(CorrTest$conf.int[2])`)\), \(p=`r fmtNumM(CorrTest$p.value)`\), \(r^2=`r fmtNumM(Corr*Corr)`\).
